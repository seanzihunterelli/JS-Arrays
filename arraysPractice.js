...
function reversedLooper(letters) {
  ...
  reversedLetters.forEach(function(item) {
    alert(item);
  })
  letters.reverse();
}
...
function removeItem(myGroceryList, items) {
  if (!items) {
    return [];
  }
  else {
    myGroceryList.slice(myGroceryList.indexOf(items), myGroceryList.indexOf(items)+1);
